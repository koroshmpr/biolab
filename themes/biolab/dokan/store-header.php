...
<div class="dokan-profile-frame-wrapper">
    <div class="border-bottom border-2 border-info border-opacity-50 mb-4 profile-frame<?php echo esc_attr($no_banner_class); ?>">
        <div class="profile-info-box text-center text-lg-start profile-layout-<?php echo esc_attr($profile_layout); ?>">
            <?php if ($store_user->get_banner()) { ?>
                <img src="<?php echo esc_url($store_user->get_banner()); ?>"
                     ...
                     class="profile-info-img col-12 rounded-5 overflow-hidden">
            <?php } else { ?>
                <div class="profile-info-img dummy-image">&nbsp;</div>
            <?php } ?>

            <div class="profile-info-summery-wrapper dokan-clearfix mt-lg-n4">
                <div class="profile-info-summery ms-lg-5 mt-lg-n5">
                    <div class="profile-info-head mb-3">
                        <div class="profile-img <?php echo esc_attr($profile_img_class); ?>">
                            <img src="<?php echo esc_url($store_user->get_avatar()); ?>"
                                 alt="<?php echo esc_attr($store_user->get_shop_name()); ?>"
                                 class="img-thumbnail rounded-5 mt-2 mb-5 my-lg-0"
                                 size="150" width="150">
                        </div>
                    ...
                    <div class="profile-info">
                        <?php if (!empty($store_user->get_shop_name()) && 'default' !== $profile_layout) { ?>
                            <h1 class="store-name fs-5 fw-bold">
                                <?php echo esc_html($store_user->get_shop_name()); ?>
                                <?php apply_filters('dokan_store_header_after_store_name', $store_user); ?>
                            </h1>
                        <?php } ?>

                        <ul class="dokan-store-info list-unstyled d-lg-flex gap-3 flex-wrap align-items-center col-md-12 col-6 mx-auto text-start">
